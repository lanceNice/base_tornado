...
from tools.base_util import get_dict_args
...
class BaseService(object):
    ...
    async def like_query(self, *args, **kwargs):
    ...
    async def list_count(self, **kwargs):
        """
        查询总数
        :param kwargs: 查询的条件
        :return:
    ...
    async def sync_execute_sql(self, sql):
        cursor = self.read_mysql_con.execute_sql(sql)
        result = []
        if cursor.description:
            column_names = [column.name for column in cursor.description]
            res = []
            for row in cursor.fetchall():
                res.append(api.base.Row(zip(column_names, row)))
            cursor.close()
            result = [var for var in res]
        return result
    ...
    def sync_execute_fetch_all(self, sql):
        db = self.read_mysql_con
        cursor = db.cursor()
        cursor.execute(sql)
        result = cursor.fetchall()
        data = self.format_result(cursor, result)
        return data

    def sync_execute_update(self, sql):
        db = self.write_mysql_con
        cursor = db.cursor()
        cursor.execute(sql)
        db.commit()
        cursor.close()
